...
// Definiamo un emit per comunicare al padre quale pagina è stata cliccata
const emit = defineEmits(['menu-click'])

const activeIndex = ref(0) // 0 = Dashboard attiva di default

// Funzione per gestire il click
const selectItem = (index: number) => {
  activeIndex.value = index
  emit('menu-click', menuItems.value[index].label)
}

// Configurazione delle voci di menu con icone SVG fedeli al mockup
const menuItems = ref([
  { 
    label: 'Dashboard', 
...
/* Contenitore Sidebar */
.um-sidebar {
  width: 250px; /* Larghezza fissa come da mockup */
  background-color: #1a202c; /* Colore Dark Blue specifico della dashboard admin */
  color: var(--um-white);
  display: flex;
  flex-direction: column;
  height: 100%; /* Occupa tutta l'altezza disponibile nel padre */
}
...
.um-nav-link {
  ...
  font-size: 1.1rem;
  font-weight: 500;
  border-left: 4px solid transparent; /* Bordo invisibile per layout stabile */
...
/* Stato Attivo (Stile "Dashboard" nell'immagine) */
.um-nav-link.active {
  color: var(--um-orange); /* Testo e icona arancioni */
  border-left-color: var(--um-orange); /* Bordo sinistro arancione */
  background-color: rgba(255, 255, 255, 0.05); /* Leggero sfondo */
  font-weight: 700;
}
